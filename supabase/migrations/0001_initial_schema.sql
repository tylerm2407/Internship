-- InternshipMatch Initial Schema (idempotent)
...
DROP POLICY IF EXISTS "Authenticated users can read firms" ON firms;
CREATE POLICY "Authenticated users can read firms"
    ON firms FOR SELECT
    USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Service role can manage firms" ON firms;
...
DROP POLICY IF EXISTS "Authenticated users can read postings" ON postings;
CREATE POLICY "Authenticated users can read postings"
    ON postings FOR SELECT
    USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Service role can manage postings" ON postings;
...
DROP POLICY IF EXISTS "Users can read their own fit scores" ON fit_scores;
CREATE POLICY "Users can read their own fit scores"
    ON fit_scores FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own fit scores" ON fit_scores;
CREATE POLICY "Users can insert their own fit scores"
    ON fit_scores FOR INSERT
    WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own fit scores" ON fit_scores;
CREATE POLICY "Users can update their own fit scores"
    ON fit_scores FOR UPDATE
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Service role can manage fit scores" ON fit_scores;
